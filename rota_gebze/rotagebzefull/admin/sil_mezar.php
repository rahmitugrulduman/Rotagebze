...
else if ($_GET) 
{
    ...
    $sorgu = $conn->query("SELECT * FROM sehit_mezarligi WHERE id =" . (int)$_GET['id']);
    $sonuc = $sorgu->fetch_assoc(); //sorgu çalıştırılıp veriler alınıyor
    unlink('images/'.$sonuc["foto"]); //eski dosya siliniyor.

if ($conn->query("DELETE FROM sehit_mezarligi WHERE id =".(int)$_GET['id'])) // id'si seçilen veriyi silme sorgumuzu yazıyoruz.
{
	header("location:sehit_mezarligi.php"); // Eğer sorgu çalışırsa ekle.php sayfasına gönderiyoruz.
}
}
