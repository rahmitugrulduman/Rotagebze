...
if ($_FILES["foto"]["name"] != "") {
    ...
    if ($baslik <> "" && $icerik <> "" && $hata=="") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
        ...
        if ($conn->query("UPDATE tarihi_hamamlar SET baslik = '$baslik', icerik = '$icerik', foto='$foto' WHERE id =" . $_GET['id'])) // Veri güncelleme sorgumuzu yazıyoruz.
        {
            echo "<script>window.location.replace('tarihi_hamamlar.php')</script>";
