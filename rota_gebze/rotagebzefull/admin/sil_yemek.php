...
else if ($_GET) 
{
    ...
    $sorgu = $conn->query("SELECT * FROM yoresel_yemekler WHERE id =" . (int)$_GET['id']);
    $sonuc = $sorgu->fetch_assoc(); //sorgu çalıştırılıp veriler alınıyor
    unlink('images/'.$sonuc["foto"]); //eski dosya siliniyor.

if ($conn->query("DELETE FROM yoresel_yemekler WHERE id =".(int)$_GET['id'])) // id'si seçilen veriyi silme sorgumuzu yazıyoruz.
{
	header("location:yoresel_yemekler.php"); // Eğer sorgu çalışırsa ekle.php sayfasına gönderiyoruz.
}
}
