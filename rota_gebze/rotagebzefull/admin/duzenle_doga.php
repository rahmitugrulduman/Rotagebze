...
if ($_FILES["foto"]["name"] != "") {
    ...
    if ($baslik <> "" && $icerik <> "" && $hata=="") { // Veri alanlarının boş olmadığını kontrol ettiriyoruz.
        ...
        if ($conn->query("UPDATE doga_turizmi SET baslik = '$baslik', icerik = '$icerik', foto='$foto' WHERE id =" . $_GET['id'])) // Veri güncelleme sorgumuzu yazıyoruz.
        {
            echo "<script>window.location.replace('doga_turizmi.php')</script>";
